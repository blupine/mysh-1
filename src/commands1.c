...
int do_exec(char** argv, int argc){ 
	...
	if(strcmp(argv[argc-1], "&") == 0){
		...
		bgpid = 0;
		bgpid = pid;              // get background process pid
		pthread_t p_thread;
		pthread_attr_t attr;
		pthread_attr_init(&attr);
		pthread_create(&p_thread, &attr, print_bg_terminate, (void*)NULL);  // print bg process termination msg
		...
		}																			// making full commands with argv
	}
...
void *print_bg_terminate(void* argv)     // print background process termination in thread
{
	int status;
